...
import { UNIQUE_VIOLATION_ERROR, EMPTY_RESULT_ERROR, RAISE_EXCEPTION } from '../configs/errors.js';
...
controller.logout = async (req, res, next) => {
    ...
    try {
        ...
        return sessionModel.invalidateById(session_id)
            .then(() => {
...
controller.verifyEmail = async (req, res, next) => {
    ...
    try {
        if (!token)
            throw { status: 401, message: "No verification token provided" };

        const payload = jwt.verify(token, verifySK);

        console.log(`[DEBUG] Verification token expires in ${((payload.exp * 1000 - Date.now()) / (60 * 1000)).toFixed(2)} mins`);
        ...
        if (error.name === "TokenExpiredError") {
            ...
            return res.status(403).json({ message: "Invalid verification token provided" });
        } else if (error.status) { // Custom error msg
            return res.status(error.status).json({ message: error.message });
        } else 
            return res.status(500).json({ message: "Internal server error" });
    }
