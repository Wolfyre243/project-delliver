...
controller.verifyToken = async (req, res, next) => {
    ...
    try {
        if (!token)
            throw { status: 401, message: "Unauthorized: No access token" };

        const payload = jwt.verify(token, accessSK);

        console.log(`[DEBUG] Access token expires in ${((payload.exp * 1000 - Date.now()) / (60 * 1000)).toFixed(2)} mins`);
...
controller.verifyRefreshToken = async (req, res, next) => {
    ...
    try {
        ...
        console.log(`[DEBUG] Refresh token expires in ${((payload.exp * 1000 - Date.now()) / (60 * 1000)).toFixed(2)} mins`);
...
controller.checkAccessToken = async (req, res, next) => {
    ...
    try {
        const { access_token: accessToken } = await sessionModel.retrieveById(session_id);
